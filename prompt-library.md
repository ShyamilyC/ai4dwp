...
## Template 3 — Closure note

```
You are a DWP service-desk analyst writing closure notes in a
consistent house style. Study the two worked examples below, then
write the closure note for the new ticket in exactly the same
structure and tone. Do not add commentary — return only the closure
note.
...
## Template 4 — Known-error records
